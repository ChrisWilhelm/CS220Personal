...
struct TQueueTest {
  ...
  using SQueue = Queue<std::string, false>;
  using FQueue= Queue<float, false>;
  ...
  static void PersonalTests() {
    ...
    *test << 'A' << 'a';
    ASSERT(test->_head != nullptr);
    ASSERT(test->_head->next != nullptr);
    ASSERT(test->_head->data == 'A');
    ASSERT(test->_head->next->next == nullptr);
    ASSERT(test->_head->next->data == 'a');
    delete test;
    CQueue *cq = new CQueue('A');
    cq->Enqueue('B');
    CQueue *cr = new CQueue('D');
    ...
    *cs = *cq + *cr;
    ...
    ASSERT(cs->_head != nullptr);
    ASSERT(cs->_head->next != nullptr);
    ASSERT(cs->_head->data == 'B');
    ASSERT(cs->_head->next->next != nullptr);
    ASSERT(cs->_head->next->data == 'D');
    ASSERT(cs->_head->next->next->next == nullptr);
    ASSERT(cs->_head->next->next->data == 'C');
    //ASSERT(cs->_head->next->next->next->next == nullptr);
    //ASSERT(cs->_head->next->next->next->data == 'C');
    delete cs;
    delete cr;
    ...
    *pis = *piq + *pir;
    ...
    delete pir;
    delete piq;
    SQueue* str = new SQueue();
    SQueue* str2 = new SQueue("test");
    ASSERT(str->_head == nullptr);
    ASSERT(str2->_head->data == "test");
    ASSERT(str2->_head->next == nullptr);
    delete str;
    delete str2;
  }
