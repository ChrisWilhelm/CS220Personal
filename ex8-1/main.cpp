...
int main() {
  ...
  int number = 1;
  while(cin >> word) {
    counters[word] = counters[word] + number;
  }
  ...
  for (map<string, int>::const_iterator it = counters.cbegin();
    ...
    words_by_freq[it->second].push_back(test);
  }
  ...
  for(map<int, vector<string> >::const_iterator it = words_by_freq.cbegin(); it != words_by_freq.cend(); ++it) {
