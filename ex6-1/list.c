...
// EXERCISE
// delete node after current, return deleted char
char delete_after(Node * node) {
  Node * temp = node->next;
  char remove = temp->data;
  node->next = temp->next;
  printf("%c\n", remove);
  free(temp);
  return remove;  // replace this stub

}

// EXERCISE
// delete first node, if any, return deleted char
...
int delete(Node ** list_ptr, char val) {
...
int delete_rec(Node ** list_ptr, char val)  {
