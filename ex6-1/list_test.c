...
int main() {
  ...
  free(str);
  assert(delete_front(&my_list) == 'A');
  str = strlist(my_list);
  assert(strcmp(str, "b S c H M") == 0);
  assert(length(my_list) == 5);
  free(str);

  assert(delete_after(my_list->next) == 'c');
  str = strlist(my_list);
  ...
  // test delete, delete_rec
  assert(delete(&my_list, 'S') == 1);
  str = strlist(my_list);
  assert(strcmp(str, "W N H") == 0);
  free(str);
  assert(delete(&my_list, 'W') == 1);
  assert(delete(&my_list, 'Q') == 0);
  
  assert(delete_rec(&my_list, 'W') == 0);
  assert(delete_rec(&my_list, 'H') == 1);
  str = strlist(my_list);
